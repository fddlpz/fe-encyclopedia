...
keywords:
  ...
  - 词法作用域
summary: >-
  本文围绕执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ展开，重点说明全局/函数/Eval
  上下文、作用域链、词法作用域、TDZ，并补充核心机制、代码示例、工程实践、常见误区和参考资料。
learningGoals:
  - 理解执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ的核心概念、问题背景和适用边界。
  - 掌握执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ在真实前端工程中的典型用法和验证方式。
  - 能够识别执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ相关的常见误区，并选择合适的优化或治理策略。
prerequisites: []
related:
...
- 本文围绕执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ展开，重点说明全局/函数/Eval 上下文、作用域链、词法作用域、TDZ，并补充核心机制、代码示例、工程实践、常见误区和参考资料。
- 阅读重点是把「执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ」放回 第 6 章 JavaScript 语言精要 的知识链路中理解，而不是只记忆孤立定义。
- 工程实践中需要同时关注 执行上下文、作用域、闭包、TDZ 等关键因素，并用可观察的指标或测试结果验证判断。
...
| 概念 | 说明 |
|---|---|
| 执行上下文 | 需要明确它的定义、适用范围、与相邻概念的区别，以及在代码中的可观察表现。 |
| 作用域 | 需要明确它的定义、适用范围、与相邻概念的区别，以及在代码中的可观察表现。 |
| 闭包 | 需要明确它的定义、适用范围、与相邻概念的区别，以及在代码中的可观察表现。 |
| TDZ | 需要明确它的定义、适用范围、与相邻概念的区别，以及在代码中的可观察表现。 |
...
在真实项目中落地执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ时，可以按以下顺序推进：
...
| 维度 | 关注点 | 判断标准 |
|---|---|---|
| 概念理解 | 执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ的定义、边界和相邻概念。 | 能用自己的话解释，并指出至少一个反例。 |
...
<Quiz
  question={"学习执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ时，最能体现工程化理解的是哪一项？"}
  options={[
    "只记住一个简短定义",
  ...
  correctIndex={1}
  explanation={"工程化理解不止停留在概念定义，还要能说明适用边界、验证方式和风险控制。这样才能把执行上下文与作用域：全局/函数/Eval 上下文、作用域链、词法作用域、TDZ从知识点转化为可落地的工程判断。"}
/>
